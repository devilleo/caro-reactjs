...
const ChatBox = ({ props }) => {
  const { messageOnchange, message, roomInfo, userInfo, removeMessageInBoxAfterSend } = props
  var arrDisplayMessageList = []
  var listMessage = roomInfo.message
  var lengthListMessage
  if (roomInfo.message) lengthListMessage = roomInfo.message.length
  else lengthListMessage = 0
  for (var i = 0; i < lengthListMessage; i += 1) {
    var element
    if (listMessage[i].email !== userInfo.email) {
      element = (
        <div key={i} className="d-flex justify-content-start mb-4">
          ...
          <div className="msg_cotainer">
            ...
            <span className="msg_time">{listMessage[i].time}</span>
          </div>
        </div>
      )
    } else {
      element = (
        <div key={i} className="d-flex justify-content-end mb-4">
          <div className="msg_cotainer_send">
            {listMessage[i].message}
            <span className="msg_time_send">{listMessage[i].time}</span>
          </div>
          <div className="img_cont_msg">
            <img
              alt=""
    ...
    arrDisplayMessageList.push(element)
  }
  return (
        ...
        <div className="cardChatBox card">
          <div className="card-header msg_head">
            <div className="d-flex bd-highlight">
              ...
              <div className="user_info">
                <span>Chat with {roomInfo.enemy.email}</span>
                {/* <p>1767 Messages</p> */}
              </div>
